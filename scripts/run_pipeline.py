...
def run_thermo(args):
    ...
    subprocess.Popen("minimap2 -t %s -ax sr %s/db/COVID.fa %s/%s_mapq80.fastq | samtools view -b | samtools sort -@ %s -o %s/ref_mapq80.bam -"
                     " && samtools index %s/ref_mapq80.bam"
                     % (args.threads, repo_dir, illumina_bam_dir,sample, args.threads, working_dir, working_dir), shell=True).wait()
    subprocess.Popen("minimap2 -t %s -ax sr %s/db/COVID.fa %s/%s.fastq | samtools view -b | samtools sort -@ %s -o %s/ref.bam -"
                     " && samtools index %s/ref.bam"
                     % (args.threads, repo_dir, illumina_bam_dir,sample, args.threads, working_dir, working_dir), shell=True).wait()
    ...
    subprocess.Popen("pilon --fix bases --changes --vcf --threads %s --mindepth 10 --genome "
                            "%s/db/COVID.fa --unpaired %s/ref_mapq80.bam --tracks --output %s/pilon"
                        % (args.threads, repo_dir, working_dir, working_dir,working_dir), shell=True).wait()
